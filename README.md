...
- **Market scanning**: query the Polymarket CLOB API for best ask prices on Yes/No outcomes across all active markets.
...
4. Run a scan (one-click start installs deps in a local venv and launches the bot):

```bash
./start.sh --auto false --poll-interval 30
```
